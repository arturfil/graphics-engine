...
void setup(void) {
  // allocate the required memory for the whole window
...
// orthographic projection from 3d -> 2d
...
void update(void) {
  while (!SDL_TICKS_PASSED(SDL_GetTicks(), previous_frame_time + FRAMRE_TARGET_TIME));
  ...
  for (int i = 0; i < N_MESH_FACES; i++) {
    ...
    // loop all three vertices of current face and applu transformations
    for (int j = 0; j < 3; j++) {
    ...
    // save the projected triangle inthe array of triangles to render
...
int main() {
  // assigning bool var to init window  
